...
const testTapaRoutes = require('./routes/test-tapa');
...
const saisieRoutes = require('./routes/saisie');
const journalRoutes = require('./routes/journal');
const actualitesRoutes = require('./routes/actualites');
const fondsOpcvmRoutes = require('./routes/fondsOpcvm');
const portefeuilleVirtuelRoutes = require('./routes/portefeuilleVirtuel');
const reservationsPresentielRoutes = require('./routes/reservationsPresentiel');
...
app.use(helmet({
  contentSecurityPolicy: false, // désactivée pour ne pas casser les scripts inline existants du site
  // Le réglage par défaut de helmet ("no-referrer") empêche le navigateur d'envoyer l'information
  // "referer" — or YouTube en a besoin pour lire les vidéos intégrées (formation.html), sinon il
  // affiche l'erreur 153. "strict-origin-when-cross-origin" reste sécurisé tout en la transmettant.
...
app.use('/api/test-tapa', testTapaRoutes);
...
app.use('/api/fonds-opcvm', fondsOpcvmRoutes);
app.use('/api/portefeuille-virtuel', portefeuilleVirtuelRoutes);
app.use('/api/reservations-presentiel', reservationsPresentielRoutes);
