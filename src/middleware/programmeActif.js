...
// Bloque le forum aux clients payants — réservé aux comptes gratuits, qui n'ont pas
// d'accompagnement personnel et bénéficient d'un espace d'échange entre pairs à la place.
async function requireCompteGratuit(req, res, next) {
  const result = await pool.query('SELECT programme FROM users WHERE id = $1', [req.user.id]);
  if (result.rows[0]?.programme !== 'gratuit') {
    return res.status(403).json({ error: "Le forum est réservé aux comptes gratuits." });
  }
  next();
}

module.exports = { requireProgrammeActif, requireCompteNonGratuit, requireCompteGratuit };
