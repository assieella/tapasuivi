...
async function genererAnalysesPortefeuilleMensuelles() {
  ...
  try {
    ...
    for (const client of clients.rows) {
      ...
      // Pas de blocage si aucune transaction — un client qui démarre tout juste mérite aussi
      // son message mensuel, avec de vraies pistes de premier achat plutôt que rien du tout.
      ...
      const profilResult = await pool.query('SELECT profile_type, revenu_mensuel FROM investor_profiles WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1', [client.id]);
      ...
      const paragraphes = genererAnalysePortefeuille({
        ...
        catalogueTitres, profileType: profil.profile_type, revenuMensuel: profil.revenu_mensuel
      });
...
async function envoyerRapportsHebdomadaires() {
  try {
    ...
    const clients = await pool.query(
      `SELECT id, prenom, full_name, email, dernier_rapport_hebdo_le FROM users WHERE role = 'client' AND programme != 'gratuit'`
    );
