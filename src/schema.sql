...
-- Confirmation d'email pour les comptes créés en libre-service (inscription gratuite) —
-- garantit que l'adresse email est réelle, avant de compter dessus pour les notifications.
ALTER TABLE users ADD COLUMN IF NOT EXISTS email_confirme BOOLEAN NOT NULL DEFAULT true;
...
-- Nettoie les doublons déjà existants (garde uniquement la ligne la plus récente par email),
-- puis empêche que ça se reproduise — une même personne ne peut plus créer qu'une seule fiche.
DELETE FROM leads_decouverte a USING leads_decouverte b
  WHERE a.id < b.id AND a.email = b.email;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'leads_decouverte_email_unique'
  ) THEN
    ALTER TABLE leads_decouverte ADD CONSTRAINT leads_decouverte_email_unique UNIQUE (email);
  END IF;
END $$;

-- Forum de discussion — réservé aux comptes gratuits (pas de conseillère personnelle,
-- le forum leur donne un espace d'échange entre pairs à la place).
...
CREATE INDEX IF NOT EXISTS idx_forum_reponses_sujet ON forum_reponses(sujet_id);

-- Actualités financières (macro-économiques UEMOA/BCEAO) — distinctes des actualités
-- boursières (marché BRVM). Même principe : brouillon généré par IA, jamais publié
-- automatiquement, toujours relu et validé par TAPA CONSEIL.
CREATE TABLE IF NOT EXISTS actualites_financieres (
  id SERIAL PRIMARY KEY,
  titre TEXT NOT NULL,
  contenu TEXT NOT NULL,
  valide BOOLEAN NOT NULL DEFAULT false,
...
-- Messages vocaux — pour les clients qui préfèrent ou ont besoin de parler plutôt que
-- d'écrire (accessibilité). L'audio est stocké encodé en base64 directement dans "contenu",
-- Railway ne conservant pas les fichiers uploadés de façon persistante.
ALTER TABLE messages ADD COLUMN IF NOT EXISTS type_message TEXT NOT NULL DEFAULT 'texte';
