...
const { accesFormationInclus } = require('../services/programme');
const router = express.Router();

// Un compte "suivi seul" (formation déjà suivie ailleurs) n'a pas accès aux modules —
// seulement au suivi de portefeuille et aux autres outils de la plateforme.
async function requireFormationIncluse(req, res, next) {
  const result = await pool.query('SELECT programme FROM users WHERE id = $1', [req.user.id]);
  if (!accesFormationInclus(result.rows[0]?.programme)) {
    return res.status(403).json({ error: "La formation n'est pas incluse dans votre formule. Contactez-nous si vous souhaitez en discuter.", formation_non_incluse: true });
  }
  next();
}

router.get('/', requireAuth, requireProgrammeActif, requireFormationIncluse, async (req, res) => {
...
router.get('/:id', requireAuth, requireProgrammeActif, requireFormationIncluse, async (req, res) => {
  const { id } = req.params;
...
router.post('/:id/repondre', requireAuth, requireProgrammeActif, requireFormationIncluse, async (req, res) => {
  const { id } = req.params;
