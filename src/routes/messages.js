...
router.get('/', requireAuth, requireProgrammeActif, requireCompteNonGratuit, async (req, res) => {
  const result = await pool.query('SELECT id, sender, contenu, type_message, created_at FROM messages WHERE client_id = $1 ORDER BY created_at ASC', [req.user.id]);
...
router.post('/', requireAuth, requireProgrammeActif, requireCompteNonGratuit, async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO messages (client_id, sender, contenu, type_message, lu_par_admin) VALUES ($1, 'client', $2, $3, false) RETURNING *`,
    [req.user.id, contenu.trim(), type]
  );
