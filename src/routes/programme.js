...
const { computeProgrammeStatus, accesFormationInclus } = require('../services/programme');
...
router.get('/status', requireAuth, async (req, res) => {
  const result = await pool.query('SELECT created_at, programme, email_confirme FROM users WHERE id = $1', [req.user.id]);
  ...
  res.json({ ...statut, programme: user.programme, formation_incluse: accesFormationInclus(user.programme), email_confirme: user.email_confirme, lien_google_calendar });
});
