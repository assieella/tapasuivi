...
const { notifierNouveauContenu } = require('../services/resend');

async function obtenirEmailsClients() {
  const result = await pool.query("SELECT email FROM users WHERE role = 'client'");
...
router.get('/clients', requireAdminAuth, async (req, res) => {
  const result = await pool.query(`
    ...
    WHERE u.role = 'client' AND u.programme != 'gratuit'
    ORDER BY u.created_at DESC
  `);
  res.json({ clients: result.rows });
});

// Liste séparée des comptes gratuits — jamais mélangés avec les vrais clients payants,
// pour éviter toute confusion. Vue simple, pas de messagerie ni d'analyse pour eux.
router.get('/comptes-gratuits', requireAdminAuth, async (req, res) => {
  const result = await pool.query(
    `SELECT id, full_name, email, phone, created_at, email_confirme FROM users WHERE role = 'client' AND programme = 'gratuit' ORDER BY created_at DESC`
  );
...
router.get('/clients/:id/messages', requireAdminAuth, async (req, res) => {
  const { id } = req.params;
  const result = await pool.query('SELECT id, sender, contenu, type_message, created_at FROM messages WHERE client_id = $1 ORDER BY created_at ASC', [id]);
...
router.post('/clients/:id/messages', requireAdminAuth, async (req, res) => {
  ...
  const type = type_message === 'audio' ? 'audio' : 'texte';
  const result = await pool.query(`INSERT INTO messages (client_id, sender, contenu, type_message, lu_par_client) VALUES ($1, 'conseiller', $2, $3, false) RETURNING *`, [id, contenu.trim(), type]);
  ...
  pool.query('SELECT email, prenom, full_name FROM users WHERE id = $1', [id]).then(async (r) => {
    ...
    // Un message vocal ne doit jamais être embarqué brut dans l'email (ce serait un énorme
    // bloc de texte en base64, illisible et risquant de faire échouer l'envoi) — on affiche
    // juste qu'un message vocal est arrivé, à écouter directement dans l'espace client.
    const contenuEmail = type === 'audio'
      ? `<p style="font-style:italic; color:#555;">🎙️ Un nouveau message vocal vous attend.</p>`
...
// Crée un nouveau titre — utile notamment pour une introduction en bourse récente, dont le
// ticker n'existe pas encore et serait sinon ignoré lors du collage des cours du jour.
router.post('/titres', requireAdminAuth, async (req, res) => {
  const { ticker, nom, secteur, pays } = req.body;
  if (!ticker || !ticker.trim() || !nom || !nom.trim()) {
    return res.status(400).json({ error: 'Le ticker et le nom sont obligatoires.' });
  }
  const tickerMaj = ticker.trim().toUpperCase();
  const existe = await pool.query('SELECT 1 FROM titres_brvm WHERE ticker = $1', [tickerMaj]);
  if (existe.rows.length) return res.status(409).json({ error: `Le titre ${tickerMaj} existe déjà.` });

  const result = await pool.query(
    `INSERT INTO titres_brvm (ticker, nom, secteur, pays, fiche_complete) VALUES ($1, $2, $3, $4, false) RETURNING *`,
    [tickerMaj, nom.trim(), secteur || null, pays || null]
  );
...
router.post('/titres/:ticker/etudes', requireAdminAuth, async (req, res) => {
  ...
  try {
    const destinataires = await obtenirEmailsClients();
    await notifierNouveauContenu({
      destinataires,
      sujet: `Nouvelle étude disponible — ${ticker}`,
      titre: `📚 Nouvelle étude : ${titre}`,
      texte: `Une nouvelle étude vient d'être publiée sur ${ticker} — consultez-la directement dans votre espace.`,
      lien: `${process.env.APP_URL || ''}/titre.html?ticker=${ticker}`
    });
  } catch (e) { console.error('Échec de la notification de nouvelle étude :', e.message); }
  ...
  if (publie === true) {
    try {
      ...
      await notifierNouveauContenu({
        ...
        titre: `📝 ${titre}`,
        texte: `Un nouvel article vient d'être publié sur le blog TAPA CONSEIL.`,
        lien: `${process.env.APP_URL || ''}/blog/${slug}`
      });
    } catch (e) { console.error('Échec de la notification de nouvel article :', e.message); }
  }
...
router.put('/blog/:id', requireAdminAuth, async (req, res) => {
  ...
  const avant = await pool.query('SELECT publie FROM articles_blog WHERE id = $1', [req.params.id]);
  const etaitDejaPublie = avant.rows[0]?.publie === true;
  ...
  // Ne notifie qu'au moment précis où l'article passe de brouillon à publié —
  // jamais à chaque petite modification d'un article déjà publié.
  if (publie === true && !etaitDejaPublie) {
    try {
      const destinataires = await obtenirEmailsClients();
      await notifierNouveauContenu({
        destinataires,
        sujet: `Nouvel article — ${result.rows[0].titre}`,
        titre: `📝 ${result.rows[0].titre}`,
        texte: `Un nouvel article vient d'être publié sur le blog TAPA CONSEIL.`,
        lien: `${process.env.APP_URL || ''}/blog/${result.rows[0].slug}`
      });
    } catch (e) { console.error('Échec de la notification de nouvel article :', e.message); }
  }
...
router.post('/clients/:id/analyse-portefeuille', requireAdminAuth, async (req, res) => {
  ...
  // Pas de blocage si aucune transaction — un client qui démarre tout juste doit aussi
  // pouvoir recevoir de vraies pistes de premier achat, pas une erreur.
  ...
  const profilResult = await pool.query('SELECT profile_type, revenu_mensuel FROM investor_profiles WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1', [clientId]);
  ...
  const paragraphes = genererAnalysePortefeuille({
    ...
    profileType: profil.profile_type, revenuMensuel: profil.revenu_mensuel
  });
...
router.post('/actualites/:id/valider', requireAdminAuth, async (req, res) => {
  ...
  try {
    ...
    await notifierNouveauContenu({
      destinataires,
      sujet: 'Nouvelle actualité du marché BRVM',
      titre: `🔍 ${result.rows[0].titre}`,
      texte: `Une nouvelle actualité du marché vient d'être publiée — consultez-la directement dans votre espace.`,
      lien: `${process.env.APP_URL || ''}/dashboard.html`
    });
  } catch (e) { console.error('Échec de la notification de nouvelle actualité :', e.message); }
...
// --- Actualités financières (veille macro-économique UEMOA/BCEAO) ---

router.post('/actualites-financieres/generer', requireAdminAuth, async (req, res) => {
  try {
    const { genererActualiteFinanciere } = require('../services/actualiteIA');
    const { contenu } = await genererActualiteFinanciere();
    const result = await pool.query(
      `INSERT INTO actualites_financieres (titre, contenu, valide) VALUES ($1, $2, false) RETURNING *`,
      [`Actualités financières — ${new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}`, contenu]
    );
    res.json({ actualite: result.rows[0] });
  } catch (err) { console.error(err); res.status(500).json({ error: err.message || "Erreur lors de la génération." }); }
});

router.get('/actualites-financieres', requireAdminAuth, async (req, res) => {
  const result = await pool.query('SELECT * FROM actualites_financieres ORDER BY created_at DESC LIMIT 30');
  res.json({ actualites: result.rows });
});

router.put('/actualites-financieres/:id', requireAdminAuth, async (req, res) => {
  const { titre, contenu } = req.body;
  const result = await pool.query(
    'UPDATE actualites_financieres SET titre = $1, contenu = $2 WHERE id = $3 RETURNING *',
    [titre, contenu, req.params.id]
  );
  res.json({ actualite: result.rows[0] });
});

router.post('/actualites-financieres/:id/valider', requireAdminAuth, async (req, res) => {
  const result = await pool.query(
    'UPDATE actualites_financieres SET valide = true, publiee_le = NOW() WHERE id = $1 RETURNING *',
    [req.params.id]
  );
  ...
  try {
    ...
    await notifierNouveauContenu({
      destinataires,
      sujet: 'Nouvelle actualité financière UEMOA',
      titre: `💹 ${result.rows[0].titre}`,
      texte: `Une nouvelle actualité financière vient d'être publiée — consultez-la directement dans votre espace.`,
      lien: `${process.env.APP_URL || ''}/actualites.html`
    });
  } catch (e) { console.error('Échec de la notification de nouvelle actualité financière :', e.message); }

  res.json({ actualite: result.rows[0] });
});

router.delete('/actualites-financieres/:id', requireAdminAuth, async (req, res) => {
  await pool.query('DELETE FROM actualites_financieres WHERE id = $1', [req.params.id]);
  res.json({ ok: true });
});
...
// Réinitialise le mot de passe d'un client — utile quand les identifiants transmis ne
// fonctionnent pas (faute de frappe lors de la création, mauvaise recopie...). Le nouveau
// mot de passe généré est renvoyé une seule fois, à copier immédiatement.
router.post('/clients/:id/reinitialiser-mot-de-passe', requireAdminAuth, async (req, res) => {
  const nouveauMotDePasse = genererMotDePasse();
  const hash = await bcrypt.hash(nouveauMotDePasse, 10);
  const result = await pool.query('UPDATE users SET password_hash = $1 WHERE id = $2 AND role = $3 RETURNING email', [hash, req.params.id, 'client']);
  if (!result.rows.length) return res.status(404).json({ error: 'Client introuvable.' });
  res.json({ email: result.rows[0].email, nouveau_mot_de_passe: nouveauMotDePasse });
});

// --- Modération du forum ---

router.get('/forum', requireAdminAuth, async (req, res) => {
  const result = await pool.query(`
    SELECT s.id, s.titre, s.contenu, s.epingle, s.created_at, u.full_name,
           (SELECT COUNT(*) FROM forum_reponses r WHERE r.sujet_id = s.id) AS nombre_reponses
    FROM forum_sujets s JOIN users u ON u.id = s.user_id
    ORDER BY s.created_at DESC
  `);
  res.json({ sujets: result.rows });
});

router.post('/forum/sujets/:id/epingler', requireAdminAuth, async (req, res) => {
  const result = await pool.query('UPDATE forum_sujets SET epingle = NOT epingle WHERE id = $1 RETURNING *', [req.params.id]);
  res.json({ sujet: result.rows[0] });
});

router.delete('/forum/sujets/:id', requireAdminAuth, async (req, res) => {
  await pool.query('DELETE FROM forum_sujets WHERE id = $1', [req.params.id]);
  res.json({ ok: true });
});

router.get('/forum/sujets/:id/reponses', requireAdminAuth, async (req, res) => {
  const result = await pool.query(
    `SELECT r.*, u.full_name FROM forum_reponses r JOIN users u ON u.id = r.user_id WHERE r.sujet_id = $1 ORDER BY r.created_at ASC`,
    [req.params.id]
  );
  res.json({ reponses: result.rows });
});

router.delete('/forum/reponses/:id', requireAdminAuth, async (req, res) => {
  await pool.query('DELETE FROM forum_reponses WHERE id = $1', [req.params.id]);
  res.json({ ok: true });
});
