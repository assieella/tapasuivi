...
router.get('/financieres', requireAuth, requireProgrammeActif, async (req, res) => {
  const result = await pool.query(
    'SELECT id, titre, contenu, publiee_le FROM actualites_financieres WHERE valide = true ORDER BY publiee_le DESC LIMIT 20'
  );
  res.json({ actualites: result.rows });
});
