async function apiCall(url, options = {}) {
  ...
  if (!res.ok) {
    // Redirection centralisée : dès qu'une page appelle l'API et que le contrat n'est pas
    // encore signé, on envoie directement vers la page de signature — pas besoin de gérer
    // ce cas dans chaque page individuellement. On évite la boucle si on y est déjà.
    if (data.contrat_non_signe && !window.location.pathname.startsWith('/contrat.html')) {
      window.location.href = '/contrat.html';
      return new Promise(() => {}); // on arrête l'exécution de la page en cours, la redirection est en route
    }
    const err = new Error(data.error || 'Une erreur est survenue.');
    err.acces_termine = data.acces_termine === true;
    err.alerte_concentration = data.alerte_concentration === true;
